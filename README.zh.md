...
- **迁移错误**：如果迁移失败，IGT 会将错误记录在 `igt_db_error.log` 中。你可以安全地删除 `.db` 文件并运行 `node tools/init-db.mjs` 从头开始（注意：这会抹除你的所有历史记录）。
...
| `lib/igt_config.json` | 是           | 模型名称、提示词（共享）       |
...
`igt.mjs`（交互主循环）在启动时会衍生一个持久化 HTTP 服务器（`lib/igt-http-server.mjs`），监听端口 `18964`。每次语法检查都是一个 HTTP POST 请求到 `http://127.0.0.1:18964/grammar`，服务器返回结构化 JSON，由客户端负责渲染输出。
