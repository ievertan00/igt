...
import { colors, paint, applyTheme } from "../ui.mjs";
...
export async function handleCommand(raw, ctx) {
  ...
  switch (cmd) {
    ...
    case "add":
    case "a": {
      const words = args.join(" ").split(",").map(w => w.trim()).filter(Boolean);
      if (!words.length) process.stdout.write(paint(colors.yellow, "Usage: /add <word1>, <word2>, …\n\n"));
    ...
    case "theme": {
      ...
      process.stdout.write("\n");
      const choice = await ctx.askLine(ctx.rl, paint(colors.gray, "Select a theme number (or press Enter to cancel) ❯ "));
      ...
      if (!isNaN(idx) && idx >= 0 && idx < themeNames.length) {
        ...
        // Refresh the UI to redraw the status bar in the new colors
