 ...
 * Also emits an OSC 11 sequence to change the terminal window background.
...
export function applyTheme(themeName) {
  ...
  if (themes[name]) {
    ...
    // OSC 10/11: set terminal foreground and background colors
    if (process.stdout.isTTY) {
      if (themes[name].termBg) {
        process.stdout.write(`\x1b]11;${themes[name].termBg}\x07`);
      } else {
        // OSC 111 resets to the terminal's own configured default background
        process.stdout.write("\x1b]111\x07");
      }
      if (themes[name].termFg) {
        process.stdout.write(`\x1b]10;${themes[name].termFg}\x07`);
      } else {
        // OSC 110 resets to the terminal's own configured default foreground
...
// Restore the terminal's default foreground and background when the process exits
if (process.stdout.isTTY) {
  process.on("exit", () => {
...
export const BOX_SEP = "\x00__BOX_SEP__\x00";
...
export function box(title, content, options = {}) {
  ...
  const titleVW = title ? visibleWidth(title) : 0;
  const top = title
    ? paint(color, "┌─ ") + title + paint(color, " " + "─".repeat(Math.max(0, width - titleVW - 5)) + "┐")
    : paint(color, "┌" + "─".repeat(width - 2) + "┐");
  const bottom = paint(color, "└" + "─".repeat(width - 2) + "┘");
  const sepLine = paint(color, "├" + "─".repeat(width - 2) + "┤");

  const lines = content.split("\n");
  const paddedLines = lines.map((line) => {
    if (line === BOX_SEP) return sepLine;
      ...
      for (const word of words) {
        if (word.length > maxWidth) {
          ...
          for (let i = 0; i < word.length; i += maxWidth)
            resultLines.push(word.slice(i, i + maxWidth));
          continue;
        }
        if (currentLine.length === 0) {
          currentLine = word;
...
// Width-aware wrap for CJK (and mixed) text. wrapText breaks on spaces and counts by
// character, but CJK prose has no spaces and each ideograph is two columns wide — so it
// would overflow the box. This breaks on character boundaries by terminal column width,
// mirroring wrapText's continuation-indent output (first line unindented so the caller's
// label sits before it; later lines indent to align under the value).
export function wrapCJK(text, maxWidth, continuationIndent = 0) {
  if (!text) return "";
  const indent = " ".repeat(continuationIndent);
  const lines = [];
  let line = "";
...
export class Spinner {
  ...
  start() {
    ...
    this.timer = setInterval(() => {
      process.stdout.write(
        `\r  ${paint(colors.cyan, FRAMES[this.idx++ % FRAMES.length])}  ${paint(colors.gray, this.message)}`,
      );
    }, 80);
  }
...
export function renderBarChart(items, { title = null, color = null, maxWidth = null } = {}) {
  ...
  const termWidth = Math.min(maxWidth || 9999, Math.max(40, (process.stdout.columns || 80) - 4));
  ...
  for (const item of items) {
    ...
    const bar =
      (color ? paint(color, "█".repeat(bars)) : "█".repeat(bars)) +
...
export function renderCompactStats(done, total, correct, wrong) {
  ...
  const dots =
    paint(colors.green, "●".repeat(filled)) + paint(colors.gray, "○".repeat(barW - filled));
...
export function renderStatusBar(stats, message, rows, scrollOffset = 0) {
  ...
  if (visibleWidth(left) + visibleWidth(right) + 2 > realCols) {
    ...
    statsLine =
      left + " ".repeat(Math.max(0, cols - visibleWidth(left) - visibleWidth(right))) + right;
  }
  ...
  const statusBg = colors.statusBg || colors.bgWhite;
  const statusFg = colors.statusFg || colors.black;
  const line1 = statusBg + statusFg + statsLine + "\x1b[K" + colors.reset;
  ...
  if (totalW <= cols) {
    line2 =
      paint(colors.cyan, msgPart) + (authorPart ? paint(colors.gray, authorPart) : "") + "\x1b[K"; // EL trick again (uses default background)
  } else {
