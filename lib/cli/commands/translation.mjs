...
export async function runTrans(text, ctx) {
  ...
  let resp = null;
  try {
    resp = await api.callTranslation(text, controller.signal);
  } catch (err) {
    if (!cancelled) {
  ...
  process.stdout.write(`${paint(colors.bold + colors.gray, "中文")}\n`);
  process.stdout.write(`${paint(colors.gray, text)}\n`);
  process.stdout.write("\n");
  process.stdout.write(`${paint(colors.bold + colors.magenta, "Translation")}\n`);
