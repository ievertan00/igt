...
export async function runAsk(_args, ctx, options = {}) {
  ...
  while (true) {
    ...
    if (isFirstTurn && options.initialPayload) {
      ...
      const raw = await ctx.askLine(ctx.rl, `${paint(colors.cyan, "[user] ❯")} `);
      if (raw === null) { aborted = true; break; } // Ctrl+C → leave the ask loop
      q = raw.trim();
      payload = q;
    }
    ...
    if (more === null) { aborted = true; break; } // Ctrl+C → discard & exit
    if (!isNo(more)) continue;

    const save = await ctx.askLine(ctx.rl, paint(colors.gray, "Save to Vault? [Y/n] "));
    if (save !== null && isYes(save)) { // Ctrl+C (null) → discard, don't save
