...
import { colors, paint, wrapText, renderCompactStats } from "../ui/index.mjs";
...
function renderVocabCard(vocab, mask) {
  ...
  let output = `${sb(title)}\n`;
  if (vocab.pos) output += `${sb(`  ${lbl("PoS")}${paint(colors.gray, vocab.pos)}`)}\n`;
  if (vocab.meaning) output += `${sb(`  ${lbl("Meaning")}${paint(colors.white, wrapText(vocab.meaning, FW, 12))}`)}\n`;
  if (vocab.zh) output += `${sb(`  ${lbl("中文")}${mask === "zh" ? MASK : paint(colors.green, vocab.zh)}`)}\n`;
  if (vocab.example) {
    ...
    output += `${sb(`  ${lbl("Example")}${paint(colors.cyan, wrapText(ex, FW, 12))}`)}\n`;
  }
  if (vocab.note) output += `${sb(`  ${lbl("Note")}${paint(colors.brightCyan, wrapText(vocab.note, FW, 12))}`)}`;
  return output;
}
