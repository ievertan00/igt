...
const sb = (text) => text.split("\n").map(l => `${paint(colors.gray, "│ ")}${l}`).join("\n");


function renderVocabCard(vocab, mask) {
  const FW = 48;
  const MASK = paint(colors.gray, "___");
  const unpaintedMask = "___";
  ...
  if (vocab.meaning) lines.push(`${lbl("Meaning")}${paint(colors.white, wrapText(vocab.meaning, FW, 10))}`);
  if (vocab.zh) lines.push(`${lbl("中文")}${mask === "zh" ? MASK : paint(colors.green, wrapCJK(vocab.zh, FW, 10))}`);
  if (vocab.example) {
    let ex = vocab.example;
    if (mask === "word") {
      const esc = vocab.word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      ex = ex.replace(new RegExp(`\\b${esc}\\b`, "gi"), unpaintedMask);
    }
    lines.push(`${lbl("Example")}${paint(colors.cyan, wrapText(ex, FW, 10))}`);
  }
  if (vocab.note) lines.push(`${lbl("Note")}${paint(colors.brightCyan, wrapText(vocab.note, FW, 10))}`);
  
  return box(title, lines.join("\n"), { width: 64, padding: 1, color: colors.gray });
}

export async function runReview(askLine, rl, limit, type = "all", config = null) {
  let preview;
  try {
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    const statsLine = renderCompactStats(i, cards.length, correctCount, wrongCount);
    ...
    if (isVocab) {
      ...
      if (hintLabel) process.stdout.write(`${sb(paint(colors.yellow, hintLabel))}\n`);
      process.stdout.write(`${sb(paint(colors.brightCyan, c.prompt))}\n\n`);
    }
    ...
    else process.stdout.write(`${sb(diffHighlight(c.prompt, c.answer))}\n\n`);


    let choice;
    while (true) {
      choice = await askLine(rl, paint(colors.gray, "[y] correct  [n] again   [a] audio   [v] explain   [d] delete  ❯ "));
      if (choice === null) break;
      const cmd = choice.trim().toLowerCase();
      if (cmd === "a" || cmd === "audio") {
        const audioText = isVocab ? (c.example ? `${c.word}. ${c.example}` : c.word) : c.answer;
        speak(audioText, config, { force: true }).catch(() => {});
        continue;
      }
      if (cmd === "v" || cmd === "view") {
        if (isVocab) {
          ...
          for (const part of c.explanation.split("\n").filter(l => l.trim())) {
            process.stdout.write(paint(colors.gray, wrapText(part.trim(), termCols, 2)) + "\n");
          }
  ...
  if (total > 0) {
    ...
    const finalStats = renderCompactStats(total, total, correctCount, wrongCount);
