...
import { toggle as toggleVoice } from "../tts.mjs";
...
register(["vocab", "v", "word", "w"], async (args, ctx) => {
  if (args.includes("--list") || args.includes("list")) {
    ...
    // Non-numeric arg → word or phrase lookup
    const lookupTerm = args.join(" ").toLowerCase();
    await runNode("tools/igt-vocab.mjs", ["--lookup", lookupTerm], ctx);
    process.stdout.write("\n");
  } else {
    try {
...
register(["undo", "u"], async (args, ctx) => {
  const targetPath = ctx.sessionState.lastTargetPath || ctx.config.ReviewPath || "";
  await runUndo(ctx.askLine, ctx.rl, args[0] ? parseInt(args[0], 10) : 1, targetPath, ctx.refreshStats);
});
...
register(["review", "r"], async (args, ctx) => {
  ...
  await runReview(ctx.askLine, ctx.rl, args[0] ? parseInt(args[0], 10) : 10, "grammar", ctx.config);
});
...
register(["voice"], async (_args, ctx) => {
  const on = toggleVoice(ctx.config);
  process.stdout.write(paint(colors.gray, `Voice ${on ? "enabled" : "disabled"} for /chat.\n`));
});
