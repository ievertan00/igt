...
function parseDiagnosis(output, errorTypes) {
  ...
  // Fallback: parse markdown format (e.g. **Diagnosis**: - Word Choice (Minor): ...)
...
// Word-level diff to extract a changed word pair (for vocab builder)
...
async function saveToDatabase(userInput, output) {
  try {
    ...
    // Vocab builder: extract word pairs from Word Choice / Idiomatic Expression errors
    ...
    if (vocabDiagnoses.length > 0 && sections.correction && userInput.trim() !== sections.correction.trim()) {
      ...
      if (pair) {
        ...
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `).run(inputId, pair.original, pair.better, userInput.trim(), explanation || null);
      }
    }
