...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url.startsWith("/review/due")) {
    try {
      ...
      const limit = parseInt(new URL(req.url, "http://x").searchParams.get("limit") || "10", 10);
      const cards = db.prepare(`
        SELECT c.id, c.source_type, c.source_id, c.prompt, c.answer, c.ease, c.interval_days, c.due_date, c.total_reviews, c.correct_streak,
               (SELECT GROUP_CONCAT(d.error_type, ' · ') FROM diagnoses d WHERE d.input_id = c.source_id) AS hint
        FROM srs_cards c
        WHERE c.source_type = 'input'
  ...
  if (req.method === "POST" && req.url === "/switch") {
    ...
    req.on("end", async () => {
      try {
        ...
        const newProvider = mgr.switchProvider(provider);
