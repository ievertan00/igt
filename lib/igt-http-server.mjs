...
function parseDiagnosis(output, errorTypes) {
  ...
  // Third attempt: strip prose before first '{' and after last '}'
...
function formatDisplayContent(data) {
  ...
  if (data.diagnoses && data.diagnoses.length > 0) {
    ...
    for (const d of data.diagnoses) {
      ...
      const line = `- ${type} (${severity}): ${explanation}`;
...
async function saveToDatabase(userInput, output) {
  try {
    ...
    const fallbackSections = parsed.raw ? null : extractSections(output);
    const correction = parsed.raw?.correction ?? fallbackSections?.correction ?? null;
    const refine = parsed.raw?.refine ?? fallbackSections?.refine ?? null;

    if (!parsed.raw && !correction && parsed.diagnoses.length === 0) {
      fs.appendFileSync(
        path.join(__dirname, "../igt_db_error.log"),
        `${new Date().toISOString()} WARN: all parse attempts failed for: "${userInput.slice(0, 80)}"\n`
      );
      return;
    }
    ...
    const insertInput = db.prepare(`
      ...
      VALUES (NULL, ?, ?, ?)
    `);
    const inputResult = insertInput.run(userInput, correction, refine);
...
async function streamGeminiResponse(res, userInput, systemPrompt, llm, startTime) {
  ...
  const formattedContent = parsed.raw
    ? formatDisplayContent(parsed.raw)
