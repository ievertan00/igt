...
function getFastSystemPrompt() {
  if (!fastSystemPrompt) {
    if (config.Prompts && config.Prompts.FastSystemPrompt) {
      fastSystemPrompt = config.Prompts.FastSystemPrompt;
    } else {
      fastSystemPrompt = "Act as an expert Linguistic Validator. Correct the user's English text for grammar, vocabulary, and style. Return ONLY the corrected text. Do not include explanations, markdown formatting, or quotes.";
    }
  }
...
const server = http.createServer(async (req, res) => {
  ...
  // Grammar check (parallel fast + full)
  if (req.method === "POST" && req.url === "/grammar/parallel") {
    let body = "";
    req.on("data", (chunk) => { body += chunk; });
    req.on("end", async () => {
      try {
        ...
        const fastPromise = llm.generateWithFallback(userInput, getFastSystemPrompt(), { taskType: "grammar" })
          .then(result => {
            const ms = performance.now() - fastStart;
            res.write(`data: ${JSON.stringify({ type: "fast_correction", text: result, ms: ms })}\n\n`);
            return result;
          });

        const fullStart = performance.now();
        const fullPromise = llm.generateWithFallback(userInput, getSystemPrompt(), { taskType: "grammar" })
          .then(async result => {
            const ms = performance.now() - fullStart;
            const sections = extractSections(result);
            const errorTypes = await getDbFunctions();
            const parsed = parseDiagnosis(result, errorTypes);

            res.write(`data: ${JSON.stringify({ 
              type: "full_analysis", 
              raw: result,
              correction: sections.correction,
              refine: sections.refine,
              diagnoses: parsed.diagnoses,
              rule: parsed.advice.rule,
              tip: parsed.advice.tip,
              ms: ms,
        ...
        // Coordinate with Promise.all for completion
        await Promise.all([
          fastPromise,
          fullPromise.then(result => { fullResult = result; })
        ]);
        ...
        // Save to database (only once, with the full result)
        if (fullResult) {
          setImmediate(() => saveToDatabase(userInput, fullResult));
        }
      } catch (error) {
        if (!res.headersSent) {
          res.writeHead(500, { "Content-Type": "application/json" });
