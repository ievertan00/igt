...
import { buildCloze } from "./cloze.mjs";
...
function undoLastInputs(db, n) {
  ...
  if (ids.length === 0) return { deleted_inputs: 0, deleted_diagnoses: 0, deleted_advice: 0, deleted_vocab: 0, deleted_cards: 0 };
  ...
  const tx = db.transaction(() => {
    ...
    const vocabDeleted = db.prepare(`DELETE FROM vocab WHERE input_id IN (${placeholders})`).run(...ids).changes;
    ...
    return { cardsDeleted, adviceDeleted, vocabDeleted, diagDeleted, inputsDeleted };
  });
  const r = tx();
  return {
  ...
  try {
    ...
    if (parsed.diagnoses.length > 0) {
      ...
      const insertCard = db.prepare(`
        ...
        VALUES ('cloze', ?, ?, ?, date('now'))
      `);
      const cloze = (correction && userInput.trim() !== correction.trim())
        ? buildCloze(userInput.trim(), correction.trim())
        : null;

      const insertMany = db.transaction((diagnoses) => {
        for (const d of diagnoses) {
          const r = insertDiagnosis.run(inputId, d.error_type, d.severity, d.explanation);
          if (cloze) insertCard.run(r.lastInsertRowid, cloze.prompt, cloze.answer);
        }
      });
    ...
    if (vocabDiagnoses.length > 0 && correction && userInput.trim() !== correction.trim()) {
      const pair = diffWords(userInput.trim(), correction.trim());
      if (pair) {
    ...
    if (refine && userInput.trim() !== refine.trim()) {
      const refinePairs = diffPhrases(userInput.trim(), refine.trim());
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url.startsWith("/review/due")) {
    try {
      ...
      const limit = parseInt(new URL(req.url, "http://x").searchParams.get("limit") || "20", 10);
      const cards = db.prepare(`
        SELECT id, source_type, source_id, prompt, answer, ease, interval_days, due_date, total_reviews, correct_streak
        FROM srs_cards
  ...
  if (req.method === "POST" && req.url === "/review/grade") {
    ...
    req.on("end", async () => {
      try {
        const { card_id, response } = JSON.parse(body || "{}");
        const dbPath = config.DbPath || "igt_data.db";
        const resolvedDbPath = path.isAbsolute(dbPath) ? dbPath : path.join(projectRoot, dbPath);
        const DbClass = await getDatabase();
        const db = new DbClass(resolvedDbPath);
        ...
        let judgement = exactMatch ? "exact" : null;

        if (!exactMatch) {
          // Synchronous LLM fall-through (T3): single Yes/No call to current provider.
  ...
  if (req.method === "GET" && req.url === "/stats") {
    try {
      ...
      db.close();
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ byLength, cefrTrajectory, mastery }));
    } catch (e) {
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: e.message }));
    }
...
async function nonStreamingResponse(res, llm, userInput, systemPrompt, startTime) {
  ...
  setImmediate(() => saveToDatabase(userInput, parsed));
