...
class ConfigLoader {
  ...
  mergeConfigs() {
    ...
    // Backward compatibility for IGT_OLLAMA_MODEL
    // Should override config.json values, but NOT specific env vars set above
    if (this.env.IGT_OLLAMA_MODEL) {
      const family = (merged.OllamaFamily || "gemma").toLowerCase();
      const familyTitle = family.charAt(0).toUpperCase() + family.slice(1);
      
      const flashKey = `Ollama${familyTitle}FlashModel`;
      const proKey = `Ollama${familyTitle}ProModel`;
      
      // If not specifically set by IGT_OLLAMA_GEMMA_FLASH_MODEL, then use IGT_OLLAMA_MODEL
      if (!this.env[`IGT_OLLAMA_${family.toUpperCase()}_FLASH_MODEL`]) {
        merged[flashKey] = this.env.IGT_OLLAMA_MODEL;
      }
      if (!this.env[`IGT_OLLAMA_${family.toUpperCase()}_PRO_MODEL`]) {
        merged[proKey] = this.env.IGT_OLLAMA_MODEL;
      }
      
      // Also set generic ones
      merged.OllamaFlashModel ??= this.env.IGT_OLLAMA_MODEL;
      merged.OllamaProModel ??= this.env.IGT_OLLAMA_MODEL;
    }
  ...
  applyEnvVars() {
    const envMappings = {
      ...
      IGT_OLLAMA_MODEL: "IGT_OLLAMA_MODEL",
  ...
  updateEnv(updates) {
    ...
    if (updated.IGT_OLLAMA_MODEL) {
      addEnvLine("IGT_OLLAMA_MODEL");
    }
