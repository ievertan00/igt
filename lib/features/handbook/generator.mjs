...
import { grammarRefAvailable, SEARCH_GRAMMAR_TOOL_DEF, executeGrammarSearch } from "../../db/grammar-ref.mjs";
...
export async function generateTailoredRule({ llm, config, errorType, examples, incremental, log }) {
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (grammarRefAvailable()) {
        // Tool-augmented: LLM can search grammar reference material before generating the rule.
        // Sources are silently discarded — handbook output is a file, not interactive.
        const result = await llm.generateWithTools(
          prompt, "",
          [SEARCH_GRAMMAR_TOOL_DEF], executeGrammarSearch,
          { taskType: "grammar" }
        );
        rawContent = result.content;
      } else {
        rawContent = await llm.generate(prompt, "", { taskType: "handbook", systemPrompt: "" });
      }
      const rule = { title: `${errorType} (Personalized)`, content: extractMarkdownContent(rawContent) };
      if (incremental) {
  ...
  log(`⚠️  No rule generated for ${errorType} — skipped.`);
  return null;
}
...
export async function generateOverallSummary({ llm, config, stats, errorFrequency, days }) {
  ...
  try {
    const result = await llm.generate(prompt, "", { taskType: "handbook", systemPrompt: "", timeout: 600000 });
