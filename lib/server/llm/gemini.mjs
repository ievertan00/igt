...
async function generate(input, systemPrompt, options = {}) {
  const { config, taskType = "grammar", responseSchema, timeout = 60000 } = options;
  ...
  const model = genAI.getGenerativeModel(buildModelConfig(config, taskType, systemPrompt, responseSchema), { timeout });
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    while (attempts <= MAX_RETRIES_PER_KEY) {
      try {
        ...
        const model = genAI.getGenerativeModel(buildModelConfig(config, taskType, systemPrompt, responseSchema), { timeout: 60000 });
        const result = await model.generateContent(input);
        return result.response.text().trim();
      } catch (error) {
        lastError = error;
  ...
  for (const key of keys) {
    ...
    while (attempts <= MAX_RETRIES_PER_KEY) {
      ...
      try {
        ...
        return { content: response.text().trim(), sources: dedup(accSources) };
      } catch (error) {
        lastError = error;
        attempts++;
