    ...
    while (attempts <= MAX_RETRIES_PER_KEY) {
      try {
        ...
        while (true) {
          ...
          let tCall = performance.now();
          const { toolCall, content, reasoningContent } = await callDeepseekAPIOnce(key, messages, openAITools, config, taskType, toolChoice, timeout);
          callTimings.push(performance.now() - tCall);

          if (!toolCall) {
            const answerMs = callTimings[callTimings.length - 1] ?? 0;
            const toolMs = callTimings.slice(0, -1).reduce((a, b) => a + b, 0);
            return { content: content || "", sources: dedup(sources), perf: { callCount: callTimings.length, toolMs: Math.round(toolMs), answerMs: Math.round(answerMs) } };
          }

          if (iter >= MAX_TOOL_ITERATIONS) {
            messages.push({ role: "assistant", content: null, tool_calls: [toolCall], ...(reasoningContent ? { reasoning_content: reasoningContent } : {}) });
            messages.push({
              role: "tool",
              tool_call_id: toolCall.id,
              content: "Maximum lookups reached. Provide your final answer now.",
            });
            tCall = performance.now();
            const finalContent = await callDeepseekFinalAnswer(key, messages, config, taskType, timeout);
            callTimings.push(performance.now() - tCall);
            const answerMs = callTimings[callTimings.length - 1] ?? 0;
            const toolMs = callTimings.slice(0, -1).reduce((a, b) => a + b, 0);
            return { content: finalContent, sources: dedup(sources), perf: { callCount: callTimings.length, toolMs: Math.round(toolMs), answerMs: Math.round(answerMs) } };
          }
