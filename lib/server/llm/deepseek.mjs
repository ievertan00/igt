 ...
 */
async function fetchDirect(url, { method, headers, body, localAddress, timeout = 60000 }) {
...
async function callDeepseekAPI(apiKey, messages, config, taskType = "grammar", responseFormat = null, timeout = 60000) {
 ...
 */
async function callDeepseekAPIOnce(apiKey, messages, tools, config, taskType, toolChoice = "auto") {
  const baseUrl = getApiBaseUrl(config);
  const model = getModelName(config, taskType);
  const localAddress = config.LocalAddress || process.env.IGT_LOCAL_ADDRESS;
 ...
 */
async function callDeepseekFinalAnswer(apiKey, messages, config, taskType) {
  const baseUrl = getApiBaseUrl(config);
  const model = getModelName(config, taskType);
  const localAddress = config.LocalAddress || process.env.IGT_LOCAL_ADDRESS;
    ...
    while (attempts <= MAX_RETRIES_PER_KEY) {
      try {
        ...
        while (true) {
          ...
          const { toolCall, content } = await callDeepseekAPIOnce(key, messages, openAITools, config, taskType, toolChoice);
          ...
          if (iter >= MAX_TOOL_ITERATIONS) {
            ...
            const finalContent = await callDeepseekFinalAnswer(key, messages, config, taskType);
