...
class LLMProviderManager {
  constructor(config = null) {
    ...
    this.currentProvider = config
      ? (process.env.IGT_LLM_PROVIDER || config.LLMProvider || "gemini").toLowerCase()
      : null;
  }
  ...
  async generateWithTools(userMessage, systemPrompt, toolDefs, toolExecutor, options = {}) {
    ...
    const shouldPreFetch = this.config?.PreFetchInsteadOfTools === true;

    if (TOOL_CAPABLE.has(this.currentProvider) && !shouldPreFetch) {
      const provider = this.getCurrentProvider();
    ...
    // Ollama (and any future non-tool provider, or when pre-fetch is explicitly configured): pre-retrieve top chunks and inject
    ...
    return { content, sources: preFetch.sources || [] };
  }
}
