...
async function getLLMManager() {
  if (!_llmManager) {
    ...
    _llmManager = initializeLLMProviders();
...
registerAskRoutes({ getLLMManager });
...
server.listen(PORT, HOST, () => {
  ...
  if (!grammarRefAvailable()) {
    console.error(`[IGT-SERVER] ⚠️  Grammar reference disabled — grammar_ref.db not found. Run: node scripts/index-wikipedia.mjs`);
  } else {
    console.error(`[IGT-SERVER] 📚 Grammar book grounding active`);
  }
});

const shutdown = () => {
  console.error("[IGT-SERVER] Shutting down...");
  server.close(() => { closeAll(); process.exit(0); });
};

process.on("SIGINT", shutdown);
process.on("SIGTERM", shutdown);
process.on("uncaughtException", (e) => console.error("[IGT-SERVER] Uncaught exception:", e.message));
