...
function resolveModel(provider, taskType, config, options = {}) {
  ...
  if (!TASK_FIELD_MAP[normalizedProvider]) {
    throw new Error(`Unknown provider: "${provider}". Available: ${Object.keys(TASK_FIELD_MAP).join(", ")}`);
  }
  ...
  if (normalizedProvider === "ollama") {
    ...
    if (!model) {
      model = DEFAULT_MODELS.ollama[family]?.[normalizedTask] || 
      ...
      if ((normalizedTask === "handbook" || normalizedTask === "practice") && requireConfig) {
        throw new Error(
          ...
          `Please configure "${fieldName}" in lib/igt_config.json.\n` +
          `Recommended: ${model}`
        );
      }
    }

    return { model, field: fieldName, isDefault };
  }

  // Validate task type for other providers
  if (!TASK_FIELD_MAP[normalizedProvider][normalizedTask]) {
    throw new Error(`Unknown task type: "${taskType}" for provider "${provider}". Available: ${Object.keys(TASK_FIELD_MAP[normalizedProvider]).join(", ")}`);
  }
    ...
    if (normalizedTask === "handbook" || normalizedTask === "practice") {
      if (requireConfig) {
        throw new Error(
          ...
          `Please configure "${fieldName}" in lib/igt_config.json or run "llm setup".\n` +
