...
// 2. Parse LLM output into structured data
// Primary: Parse JSON block from output
// Fallback: Regex-based parsing for legacy/non-JSON responses
function parseDiagnosis(output) {
  ...
  // Fallback: Regex-based parsing (legacy format)
...
try {
  // Use the unified LLM manager to generate content with grammar task type
  ...
  console.error(`${providerName.toUpperCase()} Error: All API keys failed:`, error.message);
  process.exit(1);
}
