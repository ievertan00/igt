...
function switchProvider(providerName) {
  try {
    ...
    console.log(`\n✅ Switched to LLM provider: ${newProvider}`);
...
function showStatus() {
  ...
  console.log(`    Env Var: ${process.env.GOOGLE_API_KEY ? "✓ Set" : "○ Not set"}`);
  ...
  console.log(`    Env Var: ${process.env.DASHSCOPE_API_KEY || process.env.QWEN_API_KEY ? "✓ Set" : "○ Not set"}`);
  ...
  console.log(`    Env Var: ${process.env.DEEPSEEK_API_KEY ? "✓ Set" : "○ Not set"}`);
