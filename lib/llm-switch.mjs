...
function switchProvider(providerName) {
  try {
    ...
    const modelMap = {
      ...
      ollama: config.OllamaActiveModel || "phi4"
    };
...
function showStatus() {
  ...
  const modelMap = {
    ...
    ollama: {
      grammar: config.OllamaActiveModel || "phi4",
      pro: config.OllamaActiveModel || "phi4"
    }
  };
  ...
  const ollamaModel = config.OllamaActiveModel || "phi4";
  const ollamaUrl = config.OllamaBaseUrl || "http://localhost:11434/v1";
  const isOllamaActive = current === "ollama";
  console.log(`\n  Ollama (local)${isOllamaActive ? " (ACTIVE)" : ""}:`);
