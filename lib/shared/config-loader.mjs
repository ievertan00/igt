...
class ConfigLoader {
  ...
  _merge() {
    ...
    if (merged.PreFetchInsteadOfTools !== undefined) {
      ...
      merged.PreFetchInsteadOfTools = false;
    }
    ...
    if (merged.VaultDir) {
      ...
      merged.AskPath        = path.join(merged.VaultDir, merged.AskFile);

      if (!merged.AskDir) {
        const pathConsultantions = path.join(merged.VaultDir, "Consultantions");
        const pathConsultations = path.join(merged.VaultDir, "Consultations");
        const pathAsk = path.join(merged.VaultDir, "Ask");

        if (fs.existsSync(pathConsultantions)) {
          merged.AskDir = pathConsultantions;
        } else if (fs.existsSync(pathConsultations)) {
          merged.AskDir = pathConsultations;
        } else if (fs.existsSync(pathAsk)) {
          merged.AskDir = pathAsk;
        } else {
          // Default to Consultantions directory as used in the user's vault
          merged.AskDir = pathConsultantions;
        }
      }
    }
  ...
  saveConfig(config) {
    ...
    for (const k of [
      ...
      "VocabFile", "Theme",
      "VocabularyPath", "PracticePath", "AskPath",
