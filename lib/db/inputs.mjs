...
export async function undoLastN(n) {
  ...
  const tx = db.transaction(() => {
    // Pre-count cascaded rows for the response message
    const diagCount = db.prepare(`SELECT COUNT(*) c FROM diagnoses WHERE input_id IN (${placeholders})`).get(...ids).c;
    const adviceCount = db.prepare(`SELECT COUNT(*) c FROM advice WHERE input_id IN (${placeholders})`).get(...ids).c;
    // srs_cards has a polymorphic FK — no CASCADE available, so delete explicitly
    const cardsDeleted = db.prepare(
      `DELETE FROM srs_cards WHERE source_type = 'input' AND source_id IN (${placeholders})`
    ).run(...ids).changes;
    // Delete inputs; ON DELETE CASCADE handles diagnoses and advice
    const inputsDeleted = db.prepare(`DELETE FROM inputs WHERE id IN (${placeholders})`).run(...ids).changes;
    return { cardsDeleted, diagCount, adviceCount, inputsDeleted };
  });
  const r = tx();
  return {
    deleted_inputs: r.inputsDeleted,
    deleted_diagnoses: r.diagCount,
    deleted_advice: r.adviceCount,
