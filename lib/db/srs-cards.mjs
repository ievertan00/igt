...
export async function getDueCards({ limit = 10, type = "all" } = {}) {
  ...
  return db.prepare(`
    SELECT c.id, c.source_type, c.source_id, c.prompt, c.answer, c.ease, c.interval_days,
           ...
           c.word, c.pos, c.zh, c.meaning, c.example, c.note,
           CASE WHEN c.source_type = 'input'
             THEN (SELECT GROUP_CONCAT(d.error_type, ' · ') FROM diagnoses d WHERE d.input_id = c.source_id)
             ELSE NULL
           END AS hint,
           CASE WHEN c.source_type = 'input'
             THEN (SELECT GROUP_CONCAT(d.explanation, '\n') FROM diagnoses d WHERE d.input_id = c.source_id)
             ELSE NULL
           END AS explanation
    FROM srs_cards c
...
export async function insertVocabCard({ word, pos = "", zh = "", meaning = "", example = "", note = "" }) {
  const db = await getDb();
  db.prepare(`
    INSERT INTO srs_cards
      (source_type, source_id, prompt, answer, due_date, word, pos, zh, meaning, example, note)
    VALUES ('vocab', NULL, ?, ?, date('now'), ?, ?, ?, ?, ?, ?)
  `).run(word, word, word, pos, zh, meaning, example, note);
}
