...
export function renderStatusBar(stats, message, rows, scrollOffset = 0) {
  ...
  let statsLine = left + " ".repeat(Math.max(0, cols - visibleWidth(left) - visibleWidth(right))) + right;
  
  if (visibleWidth(statsLine) > cols) {
    let truncated = "";
    let currW = 0;
    for (const char of statsLine) {
      const cw = visibleWidth(char);
      if (currW + cw > cols) break;
      truncated += char;
      currW += cw;
    }
    statsLine = truncated;
  ...
  let line2;
  if (totalW <= cols) {
    const padLen = Math.max(0, cols - totalW);
    line2 =
    ...
    line2 += " ".repeat(Math.max(0, cols - currentW));
  }

  // Use a strictly safe row index (never beyond terminal height)
  const r1 = Math.min(realRows - 1, realRows);
  const r2 = realRows;
