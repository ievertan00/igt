...
export function renderStatusBar(stats, message, rows) {
  ...
  const statsLine = left + " ".repeat(Math.max(0, cols - left.length - right.length)) + right;
  const line1 = paint(colors.bgBlue + colors.white, statsLine);

  // Line 2: Message (Interactive sentence)
  const msgLine = ` ${message}`.padEnd(cols);
  const line2 = paint(colors.italic + colors.gray, msgLine);

  // Draw at bottom
  process.stdout.write(ansi.saveCursor);
  process.stdout.write(ansi.moveBottom(rows));
