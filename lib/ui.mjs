...
export function renderStatusBar(stats, message, rows) {
  ...
  const padLen = Math.max(0, cols - visibleWidth(msgPart) - visibleWidth(authorPart));
  const line2 =
    paint(colors.cyan, msgPart) +
    (authorPart ? paint(colors.gray, authorPart) : "") +
    " ".repeat(padLen);

  // Scrolling region is 1..rows-2; lines rows-1 and rows are protected.
  // Save/restore cursor only — never toggle visibility.
  return (
    "\x1b7" +     // Save cursor
    `\x1b[${rows - 1};1H` + line1 +
    `\x1b[${rows};1H` + line2 +
