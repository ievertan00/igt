...
import { insertVocabCard, vocabCardExistsForWord, deleteLegacyVocabCards } from "../db/srs-cards.mjs";
...
function makeVocabPrompt(e) {
  return ["VOCAB", e.word, e.pos, e.zh, e.meaning, e.example, e.note].join("|||");
}
  ...
  register("POST", "/vocab/seed", async (req, res) => {
    ...
    for (const e of entries) {
      ...
      if (!(await vocabCardExistsForWord(e.word))) {
        await insertVocabCard(makeVocabPrompt(e), e.word);
        seeded++;
      }
    }
    res.writeHead(200, { "Content-Type": "application/json" });
