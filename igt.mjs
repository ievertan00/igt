...
async function startServer() {
  ...
  serverProcess = spawn(
    process.execPath,
    [path.join(__dirname, "lib", "igt-http-server.mjs")],
    {
      env: {
        ...process.env,
        IGT_SERVER_PORT: String(SERVER_PORT),
        IGT_SERVER_HOST: SERVER_HOST,
      },
  ...
  serverProcess.on("error", (e) =>
    process.stderr.write(`Server spawn error: ${e.message}\n`),
  );
  ...
  while (Date.now() < deadline) {
    ...
    if (await ping()) {
      process.stdout.write(
        `${paint(colors.gray, `● server  port ${SERVER_PORT}`)}\n\n`,
      );
      return true;
    }
  }
...
function renderResponse(data) {
  ...
  if (Array.isArray(data.diagnoses) && data.diagnoses.length) {
    const lines = data.diagnoses.map(
      (d) =>
        `- ${d.error_type || "Error"} (${d.severity || "Minor"}): ${d.explanation || ""}`,
    );
...
function logResult(targetPath, text, data) {
  ...
  try {
    ...
    process.stdout.write(
      paint(colors.yellow, "Warning: Could not log entry.\n"),
    );
  }
}
...
async function runGrammarCheck(text, targetPath) {
  ...
  process.stdout.write("\n");
  process.stdout.write(
    `${paint(colors.gray, "Input  ")}${paint(colors.white, text)}\n`,
  );
...
async function runUndo(rl, n) {
  if (!Number.isFinite(n) || n < 1) {
    process.stdout.write(
      paint(
        colors.yellow,
        "Usage: /undo [N]   (delete the last N inputs; default 1)\n\n",
      ),
    );
    return;
  }
  ...
  process.stdout.write(
    `${paint(colors.yellow, `About to delete the last ${preview.rows.length} input(s):`)}\n`,
  );
  for (const r of preview.rows) {
    const text = r.original_text.replace(/\s+/g, " ").slice(0, 80);
    process.stdout.write(
      `  ${paint(colors.gray, `#${r.id}`)}  ${paint(colors.white, text)}\n`,
    );
  }
...
function diffHighlight(original, corrected) {
  ...
  for (let i = 1; i <= n; i++)
    for (let j = 1; j <= m; j++)
      dp[i][j] =
        ow[i - 1].toLowerCase() === cw[j - 1].toLowerCase()
          ? dp[i - 1][j - 1] + 1
...
function renderVocabCard(vocab, mask) {
  ...
  const title =
    mask === "word" ? MASK : paint(colors.bold + colors.yellow, vocab.word);

  let body = "";
  if (vocab.pos) body += `  ${lbl("PoS")}${paint(colors.gray, vocab.pos)}\n`;
  if (vocab.meaning)
    body += `  ${lbl("Meaning")}${paint(colors.white, wrapText(vocab.meaning, FW, 12))}\n`;
  if (vocab.zh)
    body += `  ${lbl("中文")}${mask === "zh" ? MASK : paint(colors.green, vocab.zh)}\n`;
  if (vocab.example) {
  ...
  if (vocab.note)
    body += `  ${lbl("Note")}${paint(colors.brightCyan, wrapText(vocab.note, FW, 12))}`;
...
async function runReview(rl, limit, type = "all") {
  ...
  try {
    ...
    preview = await fetchJson(
      "GET",
      `/review/due?limit=${Math.max(1, limit)}${typeParam}`,
    );
  } catch (e) {
    process.stdout.write(paint(colors.red, `Error: ${e.message}\n\n`));
    return;
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    process.stdout.write(
      `${paint(colors.gray, `Card ${i + 1}/${cards.length}`)}\n\n`,
    );
      ...
      const hintLabel =
        hintTypes.length > 1
          ...
          : hintTypes.length === 1
            ...
            : null;
      if (hintLabel)
        process.stdout.write(`${paint(colors.yellow, hintLabel)}\n\n`);
      process.stdout.write(`${paint(colors.cyan, c.prompt)}\n`);
    }

    const enter = await askLine(
      rl,
      paint(colors.gray, "  Show Answer [Enter] ❯ "),
    );
    if (enter === null) {
      process.stdout.write("\n");
      break;
    ...
    const choice = await askLine(
      rl,
      paint(colors.gray, "  Found it? [y/n/d=delete] ❯ "),
    );
    if (choice === null) {
      process.stdout.write("\n");
      break;
    ...
    try {
      result = await fetchJson(
        "POST",
        "/review/grade",
        JSON.stringify({ card_id: c.id, correct: selfCorrect }),
      );
    } catch (e) {
      process.stdout.write(paint(colors.red, `Error: ${e.message}\n\n`));
      continue;
    ...
    if (selfCorrect) {
      correctCount++;
      process.stdout.write(
        `${paint(colors.green, "✓")} ${paint(colors.gray, `→ next due ${result.next.dueDate} (interval ${result.next.intervalDays}d)`)}\n\n`,
      );
    } else {
      wrongCount++;
      process.stdout.write(
        `${paint(colors.red, "✗")} ${paint(colors.gray, `→ reset to 1d`)}\n\n`,
      );
    }
  }
  ...
  if (total > 0) {
    ...
    process.stdout.write(
      `${paint(colors.yellow, `Reviewed ${total}, ${correctCount} correct (${pct}%), ${wrongCount} reset.`)}\n\n`,
    );
  }
}
...
async function handleCommand(raw, config, rl) {
  ...
  switch (cmd) {
    ...
    case "handbook":
    case "h":
      await runNode(rl, "tools/igt-handbook.mjs");
      process.stdout.write("\n");
      break;
    case "practice":
    case "p": {
      ...
      const nodeArgs = m
        ? [`--level=${m[1].toUpperCase()}`, `--count=${m[2]}`]
        : args;
      await runNode(rl, "tools/igt-practice.mjs", ...nodeArgs);
      process.stdout.write("\n");
      break;
    }
    case "assess":
    case "as":
      await runNode(rl, "tools/igt-assess.mjs");
      process.stdout.write("\n");
      break;
    case "add":
    case "a":
    ...
    case "v":
      if (args.includes("--list") || args.includes("list")) {
        await runNode(rl, "tools/igt-vocab.mjs", "--list");
        process.stdout.write("\n");
      } else {
        try {
          ...
          if (seed.seeded > 0)
            process.stdout.write(
              paint(
                colors.gray,
                `  Seeded ${seed.seeded} new word(s) into SRS deck.\n\n`,
              ),
            );
        } catch {}
...
function showHelp() {
  ...
  const row = (c, d) =>
    process.stdout.write(`${paint(colors.cyan, c)}${paint(colors.gray, d)}\n`);
  process.stdout.write(
  ...
  row(
    "/review    (/r)   ",
    "SRS review of cards due today (cloze + diagnoses)",
  );
  row(
    "/stats     (/st)  ",
    "Analytics dashboard: errors by hour, length, mastery",
  );
  row("/today            ", "Adaptive daily plan: SRS + practice focus area");
  row("/retry            ", "Re-run the last input with the same model");
  row(
    "/undo [N]  (/u)   ",
    "Delete the last N inputs and their diagnoses/cards (default 1)",
  );
...
async function runToday(rl, config) {
  ...
  process.stdout.write(
    `${paint(colors.gray, "1.")} ${paint(colors.white, "SRS reviews   ")}${paint(colors.cyan, `${dueCount} card(s) due`)}  ${paint(colors.gray, `(~${reviewMin} min)`)}  /review\n`,
  );
  process.stdout.write(
    `${paint(colors.gray, "2.")} ${paint(colors.white, "Cloze drill   ")}${paint(colors.cyan, `${clozeDrillCount} card(s)`)}  ${paint(colors.gray, "(~3 min)")}  /review --count=${clozeDrillCount}\n`,
  );
  process.stdout.write(
    `${paint(colors.gray, "3.")} ${paint(colors.white, "Free practice ")}${paint(colors.cyan, "1 paragraph")}  ${paint(colors.gray, "(~4 min)")}  /practice --count=1\n`,
  );
  if (focusType) {
    process.stdout.write(`${paint(colors.gray, sep)}\n`);
    process.stdout.write(
      `${paint(colors.gray, "Focus: ")}${paint(colors.magenta, focusType)}  ${paint(colors.gray, "(most frequent error — target it today)")}\n`,
    );
  }
  process.stdout.write(`${paint(colors.gray, sep)}\n\n`);

  if (dueCount > 0) {
    const go = await askLine(
      rl,
      paint(colors.gray, "  Start SRS review now? (Y/n): "),
    );
...
async function runStats() {
  let data;
  try {
    data = await fetchJson("GET", "/stats");
  } catch (e) {
    process.stdout.write(paint(colors.red, `Error: ${e.message}\n\n`));
    return;
  }
  ...
  if (data.byLength && data.byLength.length) {
    renderBarChart(
      data.byLength.map((r) => ({
        label: `${r.bucket} words`,
        value: parseFloat(r.avg_errors.toFixed(2)),
      })),
      {
        title: "Errors / input  by sentence length",
  ...
  if (data.cefrTrajectory && data.cefrTrajectory.length) {
    ...
    for (const row of data.cefrTrajectory) {
      ...
      process.stdout.write(
        `${paint(colors.gray, row.day)}  ${bar} ${paint(colors.white, row.level)}\n`,
      );
    }
    process.stdout.write("\n");
  }
  ...
  if (data.mastery && data.mastery.length) {
    ...
    for (const r of data.mastery)
      (byCat[r.mastery] || []).push(r.error_type.split(" / ").pop());
    const catColor = {
      frequent: colors.red,
    ...
    process.stdout.write(
      `${paint(colors.yellow, "Mastery  (30-day window)")}\n`,
    );

    for (const [cat, items] of Object.entries(byCat)) {
      const label = paint(catColor[cat], cat.padEnd(11));
      const countStr = paint(
        colors.gray,
        `${String(items.length).padStart(2)} type${items.length !== 1 ? "s" : " "}`,
      );
      ...
      if (cat === "frequent" || cat === "occasional") {
        ...
        for (const item of shown) {
          process.stdout.write(
            `             ${paint(colors.gray, "· ")}${paint(colors.white, item)}\n`,
          );
        }
        if (items.length > 5) {
          process.stdout.write(
            `             ${paint(colors.gray, `· … +${items.length - 5} more`)}\n`,
          );
        }
      } else {
        ...
        const more =
          items.length > 4
            ? paint(colors.gray, `  +${items.length - 4} more`)
            : "";
        process.stdout.write(
          `${label}  ${countStr}   ${paint(colors.gray, preview)}${more}\n`,
        );
      }
    }
    process.stdout.write("\n");
...
async function showSessionSummary() {
  ...
  let s;
  try {
    s = await fetchJson("GET", "/session/summary");
  } catch {
    return;
  }
  ...
  const errPerSent =
    s.total_inputs > 0 ? (s.total_errors / s.total_inputs).toFixed(1) : "0";
  const avg7 = s.avg_errors_7day.toFixed(1);
  const trend =
    s.total_inputs > 0 && s.avg_errors_7day > 0
  ...
  process.stdout.write(`${paint(colors.gray, sep)}\n`);
  process.stdout.write(
    `${paint(colors.gray, "Sentences      ")}${paint(colors.white, String(s.total_inputs))}\n`,
  );
  process.stdout.write(
    `${paint(colors.gray, "Errors/sent    ")}${paint(colors.white, errPerSent)}${paint(colors.gray, `  vs 7-day avg ${avg7}`)}${trend}\n`,
  );
  if (s.top_error)
    process.stdout.write(
      `${paint(colors.gray, "Top error      ")}${paint(colors.cyan, s.top_error)}\n`,
    );
  process.stdout.write(
    `${paint(colors.gray, "Cards added    ")}${paint(colors.white, String(s.cards_added))}  ${paint(colors.gray, `due tomorrow: ${s.cards_due_tomorrow}`)}\n`,
  );
  process.stdout.write(`${paint(colors.gray, sep)}\n\n`);
}
...
async function main() {
  ...
  process.stdout.write(
    `${paint(colors.gray, 'Usage  type text to check · /help for commands · """ for multiline')}\n`,
  );
  process.stdout.write(
    `${paint(colors.white, "──────────────────────────────────────────────────────────────────")}\n`,
  );
  ...
  process.stdout.write = (chunk, enc, cb) => {
    if (typeof chunk === "string")
      chunk = chunk.replace(/\x1b\[0?J/g, "\x1b[K");
  ...
  while (true) {
    ...
    if (text === '"""') {
      process.stdout.write(
        `${paint(colors.gray, 'multiline  ·  blank line or """ to submit · Ctrl+C to cancel')}\n`,
      );
      const lines = [];
      while (true) {
    ...
    if (text.startsWith("/")) {
      await handleCommand(text, config, rl);
      continue;
    }
