...
function extractMarkdownContent(raw) {
  ...
  // Strip outermost fenced code block (```json, ```markdown, etc.)
  const fenceMatch = raw.match(/^```(?:\w*)\s*\n([\s\S]*?)\n```\s*$/);
  if (fenceMatch) raw = fenceMatch[1].trim();

  // If the whole response is a JSON object, extract the markdown content field
  if (raw.startsWith("{")) {
    try {
...
// Strip trailing fenced code blocks that some models append to string fields
